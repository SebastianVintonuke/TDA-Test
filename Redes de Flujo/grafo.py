...
class Grafo:
    ...
    def __iter__(self):
        """
        Permite iterar sobre los vértices del grafo directamente.
        Esto hace que 'for v in grafo:' funcione correctamente.
        """
    ...
    def cambiar_peso(self, v, w, peso):
        """Cambia el peso de una arista existente."""
        if self.estan_unidos(v, w):
            self.vertices[v][w] = peso
            return True
        return False
    ...
    def copy(self):
        """Retorna una copia exacta del grafo."""
        nuevo_grafo = Grafo(dirigido=self.dirigido)
        for v, adyacentes in self.vertices.items():
            nuevo_grafo.agregar_vertice(v)
            for w, peso in adyacentes.items():
                nuevo_grafo.agregar_arista(v, w, peso)
        return nuevo_grafo