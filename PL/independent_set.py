...
def independent_set(grafo):
    ...
    problem = pulp.LpProblem('independent_set', pulp.LpMaximize)

    M = len(grafo.obtener_vertices()) + 1
    for i, v_i in enumerate(grafo.obtener_vertices()):
        ady_k = []
        ...
        problem += y[i] + pulp.LpAffineExpression([(y[k], 1) for k in ady_k]) <= 1 + M * (1 - y[i])

    problem += pulp.LpAffineExpression([(y[i], 1) for i in range(len(grafo.obtener_vertices()))])
...
class TestIndependentSet(unittest.TestCase):
    ...
    def test_estrella(self):
        g_estrella = Grafo(vertices_init=[1, 2, 3, 4, 5, 6, 7])
        g_estrella.agregar_arista(1, 2)
        g_estrella.agregar_arista(1, 3)
        g_estrella.agregar_arista(1, 4)
        g_estrella.agregar_arista(1, 5)
        g_estrella.agregar_arista(1, 6)
        g_estrella.agregar_arista(1, 7)
        print(independent_set(g_estrella))
